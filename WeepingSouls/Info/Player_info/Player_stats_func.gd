...
func set_SoulRemnants_func(SoulRemnant) -> void:
	set_text(SoulRemnant.name)
	set_level(SoulRemnant.level)
	set_max_hp(SoulRemnant.max_hp)
	set_hp(SoulRemnant.hp)
...
func set_hp(value) -> void:
	if max_hp == 1:
		pass
	hp = value
	$Update_Health/HP.text = str(int(hp)) + "/ " + str(max_hp)
	$Update_Health/Health_bar/THE_bar.points[1].x = (float(hp) / float(max_hp)) * 48.0
